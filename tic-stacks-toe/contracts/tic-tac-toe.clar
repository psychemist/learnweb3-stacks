...
;; Constants
;; The address of this contract itself
(define-constant THIS_CONTRACT (as-contract tx-sender))
;; Error thrown when a player tries to create a game with a bet amount less than the minimum (0.0001 STX)
(define-constant ERR_MIN_BET_AMOUNT u100)
;; Error thrown when a move is invalid, i.e. not within range of the board or not an X or an O
(define-constant ERR_INVALID_MOVE u101)
;; Error thrown when a game cannot be found given a Game ID, i.e. invalid Game ID
(define-constant ERR_GAME_NOT_FOUND u102)
;; Error thrown when a game cannot be joined, usually because it already has two players
(define-constant ERR_GAME_CANNOT_BE_JOINED u103)
;; Error thrown when a player tries to make a move when it is not their turn
...
;; create a new game and play move 'X' as player one. 
...
;; join an already created game as player-two
(define-public (join-game (game-id uint) (move-index uint) (move uint))
    ...
    ;; Ensure that the game being joined is able to be joined i.e. player-two is currently empty
...
;; play a turn / make a move in an ongoing game
...
;; get game from games mapping with supplied game-id
...
;; validate player's move before updating state
(define-private (validate-move (board (list 9 uint)) (move-index uint) (move uint))
    ...
    (and index-in-range x-or-o empty-spot)
))
